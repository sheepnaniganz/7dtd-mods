...
{
    //This patch is used to initialize the UI functionallity for the drop-off buttons.
    ...
    private class QS_01
    {
        public static void Postfix(XUiC_ContainerStandardControls __instance)
        {
            ...
            {
                DropOff.playerControls = __instance;

                XUiController childById = __instance.GetChildById("btnDropOff");
                if (childById != null)
                {
                    childById.OnPress += delegate (XUiController _sender, int _args)
                    {
                        DropOff.DropOffOnClick();
                    };
                }
            }
        }
    ...
    private class QS_02
    {
        public static bool Prefix(XUiC_ContainerStandardControls __instance)
        {
            ...
            {
                ValueTuple<bool, bool> valueTuple = DropOff.StashItems(srcGrid, dstInventory, Traverse.Create(__instance).Field("stashLockedSlots").GetValue<int>(), XUiM_LootContainer.EItemMoveKind.All, __instance.MoveStartBottomRight);
    ...
    private class QS_03
    {
        public static bool Prefix(XUiC_ContainerStandardControls __instance)
        {
            ...
            if (__instance.MoveAllowed(out srcGrid, out dstInventory))
            {
                DropOff.StashItems(srcGrid, dstInventory, Traverse.Create(__instance).Field("stashLockedSlots").GetValue<int>(), moveKind, __instance.MoveStartBottomRight);
            }
    ...
    private class QS_04
    {
        public static bool Prefix(XUiC_ContainerStandardControls __instance)
        {
            ...
            if (__instance.MoveAllowed(out srcGrid, out dstInventory))
            {
                DropOff.StashItems(srcGrid, dstInventory, Traverse.Create(__instance).Field("stashLockedSlots").GetValue<int>(), XUiM_LootContainer.EItemMoveKind.FillOnly, __instance.MoveStartBottomRight);
            }
    ...
    private class QS_05
    {
        public static bool Prefix(XUiC_ContainerStandardControls __instance)
        {
            ...
            if (__instance.MoveAllowed(out srcGrid, out dstInventory))
            {
                DropOff.StashItems(srcGrid, dstInventory, Traverse.Create(__instance).Field("stashLockedSlots").GetValue<int>(), XUiM_LootContainer.EItemMoveKind.FillAndCreate, __instance.MoveStartBottomRight);
            }
    ...
    private class QS_06
    {
        public static bool Prefix(XUiC_ContainerStandardControls __instance)
        {
            ...
            int lockedSlots = Traverse.Create(DropOff.playerControls).Field("stashLockedSlots").GetValue<int>();
    ...
    private class QS_07
    {
        ...
        {
            DropOff.backpackWindow = __instance;
            DropOff.playerBackpack = Traverse.Create(__instance).Field("backpackGrid").GetValue() as XUiC_Backpack;
            XUiController[] slots = DropOff.playerBackpack.GetItemStackControllers();
            ...
            for (int i = 0; i < slots.Length; ++i)
            {
                ...
                {
                    for (int j = 0; j < DropOff.dropOffLockHotkeys.Length; j++)
                    {
                        if (!UICamera.GetKey(DropOff.dropOffLockHotkeys[j]))
                            return;
                    }
                    ...
                    {
                        Traverse.Create(itemStack).Field("lockType").SetValue(DropOff.customLockEnum);
                        itemStack.RefreshBindings();
                    }
                    else if (Traverse.Create(itemStack).Field("lockType").GetValue<int>() == DropOff.customLockEnum)
                    {
    ...
    //This is used in the xml file to make the drop-off icon visible only when the player inventory is open.
    ...
    private class QS_09
    {
        ...
        {
            if (Traverse.Create(__instance).Field("lockType").GetValue<int>() == DropOff.customLockEnum)
                Traverse.Create(__instance).Field("selectionBorderColor").SetValue(new Color32(255, 0, 0, 255));
        }
    }

    //Drop-Off functionallity by pressing hotkeys (useful if other mods remove the UI buttons)
    [HarmonyPatch(typeof(GameManager), "UpdateTick")]
    private class QS_10
    {
        public static void Postfix(EntityPlayerLocal __instance)
        {
            if (UICamera.GetKeyDown(DropOff.dropOffHotkeys[DropOff.dropOffHotkeys.Length - 1]))
            {
                for (int i = 0; i < DropOff.dropOffHotkeys.Length - 1; i++)
                {
                    if (!UICamera.GetKey(DropOff.dropOffHotkeys[i]))
                        return;
                }
